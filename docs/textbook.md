...
> （凍結扱い）です。その後 PR #11（2a35ce9）で persist 規律（C1/C2/C4）が、別 PR で圧縮まわり
> （A 群の大半）が修正されるなど、安全性関連の記述には古くなったものが残ります。
> **2026-07-22 に、リースベース読みの撤去 → 当選時 no-op + ReadIndex 化（commit `60fd631` /
> `b3b21a4`）に伴い、読み取り経路・no-op・グループ D（D1〜D5）の節のみ現行コードへ更新**しました。
> それ以外（A・B・C・E 群）の節は 9a90cf5 当時のままです。**最新の問題ステータスは必ずリポジトリ
> 直下の KNOWN_ISSUES.md を参照すること。** また本書が指摘した「ドキュメント 3 世代混在」は
> 2026-07-21 のドキュメント整備で解消済み（巻末の後記参照）。
...
> 💡 **やさしく言うと**: この表は本書執筆時点（9a90cf5）の状態を基本に、読み取り経路とグループ D（D1–D5）だけを現行へ更新したものです。B1 とグループ D は修正済み。A・B2・C・E 群の行は当時のまま（実際には多くが別 PR で修正済み）なので、最新の正確なステータスはリポジトリ直下の KNOWN_ISSUES.md を参照してください。
