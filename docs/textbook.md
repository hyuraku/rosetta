...
> まま**残しています（当時のコードを解説する歴史的教材として保持）。その後 PR #11–#18 で persist
> 規律（C1/C2/C4）・圧縮まわり（A 群の大半）・読み取り経路（グループ D）などが修正されました。
> **末尾の「safety-review-2026-07-07.md の現在値」表だけは、現在の KNOWN_ISSUES.md に完全同期済み**
> です（読み取り経路・no-op・グループ D の節も現行コードに追随）。
> 個別の物語記述と現在ステータスに差がある場合は、リポジトリ直下の **KNOWN_ISSUES.md が正典**です。
> また本書が指摘した「ドキュメント 3 世代混在」は 2026-07-21 のドキュメント整備で解消済み
> （巻末の後記参照）。
...
## 最初に知るべきこと: ドキュメントは 3 世代が混在（→ 解消済み）
...
| ファイル | 行数 | 内容 |
|---|---|---|
| `state.go` | 324 | `RaftState`（状態・永続状態・タイマー） |
| `node.go` | 259 | `RaftNode`（イベントループ・`ReadIndex()`）+ **`MockTransport`** |
| `rpc.go` | 700 | RPC 型定義・全ハンドラ・選挙・複製・コミット |
| `log.go` | 174 | `LogEntry`・ログ操作・apply 処理 + デッドコード `LogManager` |
| `snapshot.go` | 206 | スナップショット機構（`Snapshotter` インターフェース） |
| `noop.go` | 51 | `becomeLeader`（当選時 current-term no-op 追記）・`NoOpCommand` |
| `readindex.go` | 160 | ReadIndex プロトコル（§6.4）: `ReadIndex` / `confirmLeadership` / `stepDown` |
...
3. 成功: `MatchIndex/NextIndex` 前進 + コミット判定
4. （旧: 過半数応答で read lease を更新 → **撤去済み** commit `b3b21a4`。`successCount` / `leaderConfirmed` / `majority` 引数も削除。リーダーシップ確認は読み取り時に `ReadIndex` / `confirmLeadership` がその場でハートビート 1 巡を行う方式へ）
5. 失敗: fast rollback
...
- **現 term のエントリのみ直接コミット**（§5.4.2 に忠実、`rpc.go`）。過去 term のエントリは現 term のエントリが積まれたときに間接的に確定する
- 当選時に current-term no-op を積むようになったため（D3 解消、`becomeLeader`、commit `60fd631`）、**新リーダーは就任直後にその no-op をコミットでき、前任 term のエントリもそこで間接的に確定する**（次の書き込みを待つ必要がない）

> 💡 **やさしく言うと**: 「コミット」とは、過半数のメンバーに行き渡った記録を「もう取り消せない確定事項」と宣言することです。新リーダーは就任時に空の一筆（no-op）を必ず書くので、それが行き渡った時点で前任者の記録もまとめて確定できます。
...
## read の線形化 — ReadIndex プロトコル（readindex.go, §6.4）

`ReadIndex`（`raft/readindex.go`）:

- ①リーダーかつ current-term コミット済みを確認（未達なら `ErrNoCurrentTermCommit`）②現在の `commitIndex` を readIndex として捕捉 ③`confirmLeadership` がハートビート 1 巡で過半数から current-term ACK を集め、「自分がまだ唯一のリーダー」であることを証明してから readIndex を返す
- 失敗時は stale 値を返さずエラー（`ErrNotLeader` / `ErrNoCurrentTermCommit` / `ErrLeadershipNotConfirmed`）。より高い term を見たら `stepDown`。単一ノードは自分が過半数なので即返す
- KV 側は `Get` が `ReadIndex()` → `waitForApplied(readIndex)` → `getLocal(key)` の順で読む（`kvstore/store.go`）

> 💡 **やさしく言うと**: リーダーは読み取りの前に、まず「いまこの瞬間もまだ自分がリーダーか」を仲間の過半数に一度だけ確認します。確認できた時点の帳簿の到達点まで自分の反映が追いついてから答えるので、古い値を返しません。時計に頼っていた旧方式の弱点がなくなりました。

---

## 旧 read lease の弱点はどう解消したか（D1〜D3 = 解消、commit `b3b21a4` / `60fd631`）

以前は「過半数がついさっき自分をリーダーと認めた」時刻から `electionTimeout` 以内ならローカル即答する**リース方式**（`CanServeReadOnlyQuery` / `lastLeaderConfirmation`）でしたが、以下の弱点があり **commit `b3b21a4` で機構ごと撤去**されました:

- （D1）lease 期間に**自分の**乱択 `electionTimeout` を流用しており、より短い値を引いたフォロワーが lease 有効中に当選し得た → ReadIndex は都度過半数 ACK で証明するため時計に依存しない
- （D2）lease 起点が「応答を**受信した**時刻」で通信往復ぶん実質延長されていた → 撤去済み
- （D3）当選時 no-op がなく、新リーダーが前任 term のコミット済みエントリを反映する前に lease 読みが成立し得た → `becomeLeader` の当選時 no-op（commit `60fd631`）で解消

> 💡 **やさしく言うと**: 「さっき認められたから大丈夫」という時計頼みの近道をやめ、読むたびに仲間へ一言確認する堅実な方式に切り替えました。これで「自分はまだリーダーのつもりだが、よそで新リーダーが立っていた」瞬間に古い値を返す穴がふさがりました。
...
5. 結果受信後、コミット済みは log index で解決し、ロール変化後も結果を返す（D5 解消、commit `16a9b31`）

注意: かつては手順 5 が結果**適用済み**でも term 変化だけで `"leadership lost"` エラーにし、成功した書き込みにエラーを返し得た（safety-review D5）。commit `16a9b31` でコミット済みは log index で解決するようになり解消。

> 💡 **やさしく言うと**: 書き込みは「受付番号を発行 → 多数決に回す → 自分の番号の結果が戻るのを最大 5 秒待つ」という流れです。かつては実は成功しているのに「失敗しました」と答える癖がありましたが、いまは確定済みなら正しく成功を返します。
...
## GET の経路 — ReadIndex（store.go, §6.4）

```go
func (kvs *KVStore) Get(key string) (string, error) {
    readIndex, err := kvs.raft.ReadIndex()   // 過半数で現リーダーを証明し readIndex を捕捉
    if err != nil {
        return "", err                        // 非リーダーは "not leader" → HTTP 503
    }
    if err := kvs.waitForApplied(readIndex); err != nil {  // readIndex まで適用を待つ
        return "", err
    }
    return kvs.getLocal(key)                   // ローカル map から読む
}
```

- `ReadIndex` が成功したら `waitForApplied` で `lastAppliedIndex` が readIndex に追いつくのを待ち（2ms 間隔ポーリング、`operationTimeout=5s`）、`getLocal` でローカル map を読む
- 非リーダーや過半数確認失敗時は `ReadIndex` がエラーを返し、`"not leader"` は HTTP 503 リダイレクトにマップされる（従来のクライアント挙動を維持）
- ✅ CLAUDE.md の旧「GET は状態を変えないから安全」という説明は不正確だったが、いまは ReadIndex により線形化される（旧 lease の弱点は撤去、第1章参照）

> 💡 **やさしく言うと**: 読み取りは「まず仲間の過半数に今もリーダーか確認 → 確認時点の到達点まで手元の反映を追いつかせる → ローカルから即答」という 1 本道になりました。時計頼みの近道はやめたので、古い値を返しません。
...
結論: かつては重複検出がテスト経由でしか動かず（safety-review D4）、タイムアウト後のリトライで二重適用され得たが、**commit `52afd48` で ClientID/SeqNum を PUT/DELETE 経路に配線し at-most-once 化して解消**。
...
| ファイル | 主対象 |
|---|---|
| `raft_test.go` | 状態遷移・投票・fast rollback（Conflict 3 パターン） |
| `persistence_test.go` | FileStorage の保存/復元/アトミック性（`.tmp` が残らない） |
| `duplicate_detection_test.go` | ClientID/SeqNum 機構（※Command を直接組んで検証。D4 解消後は実 API 経路も配線済み、commit `52afd48`） |
...
- ~~重複検出の HTTP 未配線（D4）~~ → 解消済み（`52afd48`、実 API 経路に ClientID/SeqNum を配線）
- ~~lease の線形化可能性（D1–D3）~~ → 解消済み（ReadIndex + 当選時 no-op に置換、`60fd631` / `b3b21a4`）
...
各指摘の現状を、現在の KNOWN_ISSUES.md に完全同期した表（本文の他の章は 9a90cf5 時点のままだが、この表のみ現行ステータス）:
...
| R14 | Joint consensus・構成変更ログエントリ・新旧 quorum の二重確認が未実装 | ✅ **修正済み**（`9da332c` / `3a82a6a` / `49e513e` / `54fba63`、構成をログエントリ化し追記時点で適用、joint は旧・新両方の過半数を要求、管理 API と snapshot 内構成を追加） |
| R16 | `SnapshotInterval` が未使用で、`LoadConfig` がゼロ値のまま `Validate` に渡す | ❌ **未修正**（P3） |
| R18 | `examples/benchmark` の read ワークロードが鍵不一致でヒットしない | ❌ **未修正**（P3） |
| R20 | learner（non-voting member）の追いつき段階が未実装で、追加直後のサーバーが即 quorum に数えられる | ❌ **未修正**（P2、R14 の修正作業中に起票） |

> 💡 **やさしく言うと**: この表は本書の他の章と違い、現在の KNOWN_ISSUES.md に同期済みです（同期時点: 2026-09-06 / commit `e183622`）。B1・B2・A1–A8・C1–C4・D1–D5 はすべて別 PR で修正済みで、2026-09-06 の再監査（`docs/raft-audit-2026-09-06.md`）で見つかった P0 の R1（Log Matching）・R2（Leader Completeness/永続化）・R3–R5（snapshot の世代整合・payload/metadata の同一世代化・古い snapshot の適用禁止）、P1 の R6（降格時のタイマー再始動）と data race 2 件（E1/E2）も修正済みです。その後 R9–R13（KV/client/API の細部と AppendEntries の境界 term 検査・commit 上限）、R15（InstallSnapshot の chunk 転送）、R17（CI と SA4011）、R19（`Kill` の goroutine join）も修正され、B3（rs.mu 保持下での applyCh 送信という liveness 問題）は専用 applier goroutine への分離で解消しました。R14（joint consensus による動的メンバーシップ）も修正済みです。**未修正は R16（設定）・R18（benchmark）・R20（learner）の 3 件**です。ただしそれは「修正済みの経路に問題が残っていないと確認できた」という意味ではないため、「Raft の安全性そのものを破る既知の経路は残っていない」とはまだ言えません。詳細は `KNOWN_ISSUES.md` を参照してください。**本文の第 12 章「InstallSnapshot RPC 受信側」にある A7 の指摘は 9a90cf5 当時の記述であり、現在は解消済み**です。
...
2. 修正順序は safety-review の推奨が妥当: **C 系（persist）→ B2 → A 系（圧縮）→ D 系（読み取り）**。B1・D 系（D1–D5）は完了済み（D 系は ReadIndex + 当選時 no-op 化）
...
- ドキュメントは 3 世代混在（→ 2026-07-21 整備で解消済み、後記参照）。**迷ったら KNOWN_ISSUES.md、最後はコード**
...
# 後記（2026-07-21 ドキュメント整備）

本書の指摘を受けて、2026-07-21 にドキュメントの全面整備を実施した。

- **3 世代混在は解消**: 第 1 世代（deployment / troubleshooting / performance）は有用部分を
  `examples/benchmark/README.md` 等へ移植のうえ削除。第 2 世代（README / CLAUDE.md /
...
- 本書自体は commit 9a90cf5 時点の記録として凍結する。安全性の最新状態は
  KNOWN_ISSUES.md が唯一の正である
