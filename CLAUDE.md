...
- **`make bench` and `go test -bench` find nothing.** There are no Go `Benchmark*` functions in this repo. Benchmarking is an external HTTP-level tool: `cd examples/benchmark && go build . && ./benchmark -url=http://localhost:9080`. The read workload uses deterministic, index-derived keys shared between preload and reads, so it is expected to hit — see `KNOWN_ISSUES.md` (R18, fixed).
- **Compaction's known safety and liveness defects on the receive path are fixed.** Group A is clear (A7, the InstallSnapshot receiver keeping a divergent suffix, is fixed in `019d33e`), and so are the three snapshot-path issues the 2026-09-06 re-audit found (`docs/raft-audit-2026-09-06.md`): cross-file generation consistency (R3, `0695b95`), metadata/payload sampled from different generations (R4, `f53617e`), and an older snapshot rolling back applied state (R5, `156510a`). The liveness gap (B3, `f873d9b`) is closed too — the handler still writes the payload under `rs.mu`, but the `applyCh` hand-off is queued for the applier instead of blocking the lock holder. Note `MaxRaftState=0` is not settable — `Validate` rejects it (`config/config.go`), despite older docs suggesting it as a workaround.
- **Election/heartbeat timing is configurable, but only through `-config`.** `config.Config`'s `ElectionTimeout`/`HeartbeatTimeout` are wired into `raft.Timing` via the functional option `raft.WithTiming` (`main.go`'s `resolveConfig` path); there are no `-election-timeout`/`-heartbeat-timeout` flags. `ElectionTimeout` is the timeout base, and the jitter added on top is the same length again ([base, 2*base) — 150-300ms at the defaults). `SnapshotInterval` and `LogLevel` remain reserved: declared in the config struct, read by nothing (`KNOWN_ISSUES.md` R16, fixed for the timing fields; these two are explicitly still unread).
...
- **Writes only go to the leader.** PUT/DELETE against a follower return HTTP 503 with redirect information. Tests that hit an arbitrary node will flake. `POST /cluster/add` and `/cluster/remove` behave the same way; `GET /cluster/config` is answered by any node.
- **`-join` is still rejected** even though membership changes exist. A server is added by starting it with the *existing* cluster's `-peers` list — so it is not a voter yet and will not campaign — and then POSTing to the leader's `/cluster/add`. `network/discovery.go`'s `ClusterManager` is not on that path at all.
- **`/cluster/add` adds a learner, not a voter.** The append is a single, non-joint configuration that leaves `Voters` alone and puts the server in `Learners`; it is replicated to but no quorum counts it (`QuorumReached` is unchanged by design). The leader promotes it itself — `promoteCaughtUpLearnerLocked`, called under `rs.mu` right after `MatchIndex` moves — and only then is there a C_old,new. So a test that adds a server must poll for it in `voters`, not just for the call to return. While a learner exists the addition is in flight: every other change returns `ErrConfigChangeInProgress`, except removing that learner. Permanent learners / read replicas are out of scope (`KNOWN_ISSUES.md` R20, fixed).
- **A `RequestVote` from a node that has a live leader is ignored, term and all** (§6). If a test needs a vote granted right after an AppendEntries, ask within the current term — a higher term is deliberately disregarded for one election timeout.
...
- **The cluster configuration is decided by log entries and takes effect the moment one is received**, committed or not (paper §6, KNOWN_ISSUES.md R14). It lives in `PersistentState.Config` and is re-derived from the log on every change, which is what makes a truncated configuration entry revert to the previous one; `SnapshotConfig` holds the configuration at the snapshot boundary for when compaction has removed every entry, and an `InstallSnapshot` carries it. Nothing decides a quorum from the `-peers` list — `ClusterConfig.QuorumReached` does, requiring a majority of both the old and the new voter set while the configuration is joint.
