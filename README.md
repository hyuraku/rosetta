...
- **Raft Consensus**: Leader election with randomized timeouts, log replication with consistency checks, and heartbeats. The two P0 findings of the 2026-09-06 re-audit on this path are fixed: the leader's append now checks leadership and appends durably under one lock (R1), and a follower no longer ACKs entries it failed to persist, not even on a resend (R2). Other Group R findings remain open — see [KNOWN_ISSUES.md](KNOWN_ISSUES.md)
